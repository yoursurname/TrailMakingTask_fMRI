...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\emilo\\OneDrive - Lund University\\Experiment paradigms\\TMT_PsychoPY_new\\TMT_fmri\\TMT_fmri_lastrun.py',
...
# Initialize components for Routine "TimeCheck"
TimeCheckClock = core.Clock()
...
# Initialize components for Routine "TimeCheck_2"
TimeCheck_2Clock = core.Clock()
...
# Initialize components for Routine "TimeCheck_3"
TimeCheck_3Clock = core.Clock()
...
Blocks_TMTControl = data.TrialHandler(nReps=1.0, method='sequential', 
    ...
    trialList=data.importConditions('TMTstop-Control_1_conditions.xlsx'),
...
for thisBlocks_TMTControl in Blocks_TMTControl:
    ...
    for thisTrials_TMTControl in trials_TMTControl:
        ...
        # ------Prepare to start Routine "TimeCheck"-------
        continueRoutine = True
        # update component parameters for each repeat
        # keep track of which components have finished
        TimeCheckComponents = []
        for thisComponent in TimeCheckComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        t = 0
        _timeToFirstFrame = win.getFutureFlipTime(clock="now")
        TimeCheckClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
        frameN = -1
        
        # -------Run Routine "TimeCheck"-------
        while continueRoutine:
            # get current time
            t = TimeCheckClock.getTime()
            tThisFlip = win.getFutureFlipTime(clock=TimeCheckClock)
            tThisFlipGlobal = win.getFutureFlipTime(clock=None)
            frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
            # update/draw components on each frame
            if globalClock.getTime() - startTime >= 15:
                Blocks_TMTControl.finished = True
                continueRoutine = False
        ...
        # -------Ending Routine "TimeCheck"-------
        for thisComponent in TimeCheckComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # the Routine "TimeCheck" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
...
Blocks_TMTA = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('TMTstop-A_1_conditions.xlsx'),
...
for thisBlocks_TMTA in Blocks_TMTA:
    ...
    for thisTrials_TMTA in trials_TMTA:
        ...
        # ------Prepare to start Routine "TimeCheck_2"-------
        continueRoutine = True
        # update component parameters for each repeat
        # keep track of which components have finished
        TimeCheck_2Components = []
        for thisComponent in TimeCheck_2Components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        t = 0
        _timeToFirstFrame = win.getFutureFlipTime(clock="now")
        TimeCheck_2Clock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
        frameN = -1
        
        # -------Run Routine "TimeCheck_2"-------
        while continueRoutine:
            # get current time
            t = TimeCheck_2Clock.getTime()
            tThisFlip = win.getFutureFlipTime(clock=TimeCheck_2Clock)
            tThisFlipGlobal = win.getFutureFlipTime(clock=None)
            frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
            # update/draw components on each frame
            if globalClock.getTime() - startTime >= 15:
                Blocks_TMTA.finished = True
                continueRoutine = False
        ...
        # -------Ending Routine "TimeCheck_2"-------
        for thisComponent in TimeCheck_2Components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # the Routine "TimeCheck_2" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
...
for thisBlocks_TMTB in Blocks_TMTB:
    ...
    for thisTrials_TMTB in trials_TMTB:
        ...
        # ------Prepare to start Routine "TimeCheck_3"-------
        continueRoutine = True
        # update component parameters for each repeat
        # keep track of which components have finished
        TimeCheck_3Components = []
        for thisComponent in TimeCheck_3Components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        t = 0
        _timeToFirstFrame = win.getFutureFlipTime(clock="now")
        TimeCheck_3Clock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
        frameN = -1
        
        # -------Run Routine "TimeCheck_3"-------
        while continueRoutine:
            # get current time
            t = TimeCheck_3Clock.getTime()
            tThisFlip = win.getFutureFlipTime(clock=TimeCheck_3Clock)
            tThisFlipGlobal = win.getFutureFlipTime(clock=None)
            frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
            # update/draw components on each frame
            if globalClock.getTime() - startTime >= 15:
                Blocks_TMTB.finished = True
                continueRoutine = False
        ...
        # -------Ending Routine "TimeCheck_3"-------
        for thisComponent in TimeCheck_3Components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # the Routine "TimeCheck_3" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
