...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\emilo\\OneDrive - Lund University\\Experiment paradigms\\TMT_PsychoPY_new\\TMT-Control\\TMTAB_lastrun.py',
...
trials = data.TrialHandler(nReps=1.0, method='sequential', 
    ...
    trialList=data.importConditions('TMT-control.xlsx'),
...
trials_2 = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('TMT-control2.xlsx'),
...
_key_resp_3_allKeys = []
# keep track of which components have finished
FinishRound1Components = [text_3, key_resp_3]
for thisComponent in FinishRound1Components:
    thisComponent.tStart = None
    thisComponent.tStop = None
    thisComponent.tStartRefresh = None
    thisComponent.tStopRefresh = None
...
t = 0
_timeToFirstFrame = win.getFutureFlipTime(clock="now")
FinishRound1Clock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1

# -------Run Routine "FinishRound1"-------
while continueRoutine:
    # get current time
    t = FinishRound1Clock.getTime()
    tThisFlip = win.getFutureFlipTime(clock=FinishRound1Clock)
    tThisFlipGlobal = win.getFutureFlipTime(clock=None)
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
