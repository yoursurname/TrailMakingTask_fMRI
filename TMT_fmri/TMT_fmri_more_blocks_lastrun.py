...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Hassl\\OneDrive - Lund University\\Biofinder\\fMRI-tasks\\TrailMakingTask_fMRI\\TMT_fmri\\TMT_fmri_more_blocks_lastrun.py',
...
Instruct = visual.ImageStim(
    win=win,
    name='Instruct', 
    image='Instructions_TMT_shorter.png', mask=None, anchor='center',
...
TMTA_stimuli_starting_rows = list(range(0, TMTA_last_row, 26))
TMTB_first_row = TMTA_stimuli_starting_rows[-1]+n_trial_images
TMTB_last_row = TMTB_first_row + TMTA_last_row
TMTB_stimuli_starting_rows = list(range(TMTB_first_row,TMTB_last_row , 26))
...
Stim = visual.ImageStim(
    ...
    ori=0.0, pos=(0, 0), size=(1.40, 0.78),
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=0.0)
...
block = data.TrialHandler(nReps=len(experiment_sequence), method='sequential', 
...
for thisBlock in block:
    ...
    # keep track of which components have finished
    trial_infoComponents = [trial_instructions]
    ...
    for thisTrial in trials:
        ...
        block.addData("TrialDuration", globalClock.getTime() - startTime)
    ...
    while continueRoutine:
        ...
        if Version == 'TMT_Control' and globalClock.getTime() - startTime >= control_time_limit:
            continueRoutine = False
        elif Version == 'TMT_A' and globalClock.getTime() - startTime >= TMT_time_limit:   
            continueRoutine = False
        elif Version == 'TMT_B' and globalClock.getTime() - startTime >= TMT_time_limit:   
            continueRoutine = False
    ...
    routineTimer.reset()
# completed len(experiment_sequence) repeats of 'block'
...
print(f"Total time: {globalClock.getTime()-expStart}")
