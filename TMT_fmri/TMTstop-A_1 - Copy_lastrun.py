...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\emilo\\OneDrive - Lund University\\Experiment paradigms\\TMT_PsychoPY_new\\TMTstop-A_1\\TMTstop-A_1 - Copy_lastrun.py',
...
# set up handler to look after randomisation of conditions etc
Condition_trials = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('TMTstop-A_1_conditions.xlsx'),
    seed=None, name='Condition_trials')
...
if thisCondition_trial != None:
    for paramName in thisCondition_trial:
        exec('{} = thisCondition_trial[paramName]'.format(paramName))
...
for thisCondition_trial in Condition_trials:
    ...
    for thisComponent in FixCrossComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    Condition_trials.addData('polygon.started', polygon.tStartRefresh)
    Condition_trials.addData('polygon.stopped', polygon.tStopRefresh)
